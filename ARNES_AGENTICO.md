...
## Regla canónica: Skill @arnes/ARNES_AGENTICO

**Este documento es el skill @arnes/ARNES_AGENTICO. Se instala en TODOS los repos.**

Al arrancar cualquier agente OpenCode, siempre usar:
```bash
...
Una skill es **cualquier conocimiento que mejora un proceso**. No necesita código ejecutable. El código es una forma de implementar la metodología, no un requisito.

**Regla de rotación de modelos (obligatoria):**
- Tier 1 (free, sin API key): `opencode/deepseek-v4-flash-free`, `opencode/ling-3.0-flash-free`, `opencode/longcat-2.0-free`, `opencode/mimo-v2.5-free`, `opencode/nemotron-3-ultra-free`, `opencode/north-mini-code-free`, `opencode/big-pickle`
- Tier 2 (GEMINI_API_KEY): `google/gemini-2.5-flash`, `google/gemini-2.5-pro`, `google/gemini-3.5-flash`
- **Nunca el mismo modelo dos veces seguidas.**

Los sub-agentes de orquestación usan el agente `hermes` y sus children:
- `hermes` (primary): deepseek-v4-flash-free → ling-3.0-flash-free
- `hermes-b2-research` (subagent): ling-3.0-flash-free → mimo-v2.5-free
- `hermes-b3-tools` (subagent): nemotron-3-ultra-free → north-mini-code-free
- `hermes-b4-multimedia` (subagent): big-pickle → longcat-2.0-free
- `hermes-b5-b6-adapters` (subagent): mimo-v2.5-free → deepseek-v4-flash-free
- `hermes-qa` (subagent): big-pickle (read-only, verificación mecánica)

Estos agentes viven en `.opencode/agents/` del repositorio `Airhonreality/skills` y se copian a cada repo que adopta el arnés. Ver `AGENTS.md` §7 (Delegación a subagentes).
...
**Regla de entrada (directriz de arnés, fuente de verdad en `AGENTS.md` → "Tipos de Input"):** una app prefabricada (carpeta + venv + tests) es un **artefacto de SALIDA (FR4)**, no un recurso de conocimiento (FR3). NO la procesa Daemon Docs ni va a `knowledge/archive/`: el escáner la detecta como directorio-app y `daemon-docs.sh` hace handoff a `daemon-apps.sh`, que la mueve íntegra a `apps/` y crea un wrapper de skill en `skills/apps/` (directriz tool real, sin leer/copiar `.venv`, `__pycache__`, caches).
...
### Skill (template, `arnes/skills/<domain>/<skill>/SKILL.md`)

**Conforme al estándar abierto agentskills.io.** Solo `name` y `description` en frontmatter
son obligatorios; el resto lo ignoran los runtimes. Estructura: `SKILL.md` + `references/`
(+ `scripts/` opcional solo si necesita código y fue aprobado por el humano).

```markdown
---
name: <slug-del-skill>
description: <qué hace en una o dos líneas de lenguaje de negocio, con palabras clave
  de activación y un "NO usar cuando..." breve para acotar el trigger>
triggers: [<keywords que disparan la skill>]      # OPCIONAL
tools_required: [<mcp/scripts que necesita>]       # OPCIONAL
allowed-tools: [<herramientas pre-aprobadas>]      # OPCIONAL: gobernanza granular
license: MIT                                        # OPCIONAL
---
```

> **Nota (A1 escritura productiva + múltiples skills de alto impacto):** ver "Homeostasis de
> skills" en `AGENTS.md`. Con catálogos grandes NO se aprueba skill por skill de humano: el
> ciclo de control automático valida el 100% determinista + smoke; solo skills de alto
> riesgo (ejecutable con acceso exterior) pasan a cola de revisión asinc. Corregir en
> `daemon-docs.sh` Fase 5a y governance QA-4 el texto de plantilla para incluir estos campos.

> **Directriz (2026-08-07) — la máquina ENVUELVE tools reales, no inventa contratos.**
> Una skill ejecutable = wrapper delgado de una tool real (CLI/SDK del servicio), auto-descriptiva
> via `--help`, determinista y componible, con credenciales solo por env var (I4). Está
> PROHIBIDO el mapeo schema-driven API→CRUD genérico (contrato de 5 verbos); el LLM nunca
> genera código ejecutable para servicios externos — solo descubre la tool y describe el wrapper
> (env interface + guardrails + comando base). El agente que quiere "revisa mi workspace de X"
> invoca `x --help`, compone la secuencia y formatea la salida; la tool hace el efecto real.

## Objetivo
<Una frase en lenguaje de negocio>

## Protocolo
<Pasos numerados de la metodología; si es largo, referencia references/METHODOLOGY.md>

## Guardrail & Restricciones
- NUNCA <X>; en su lugar <Y específico>. Los LLMs responden mejor a restricción + alternativa.

## Verificación obligatoria
<Chequeo de parada que el agente ejecuta ANTES de dar por terminado. Si no aplica: N/A.>

## Manejo de error / rollback
<Si <tool> falla: no adivinar el fix; releer references/ y hacer máximo 1 reintento>
...
## Dependencias
- <qué necesita (env vars, etc.). Nunca credenciales en texto claro>

## Fuentes
- <referencias a archive/>

## Referencias
- Ver `references/METHODOLOGY.md` para el detalle completo.
```

**Nota:** las secciones `Guardrail & Restricciones`, `Verificación obligatoria` y `Manejo
de error / rollback` son obligatorias en toda skill; la Fase 6 del daemon y `governance.sh`
las verifican. Son el mecanismo de control/determinismo sobre el comportamiento estocástico
del modelo.

### references/METHODOLOGY.md (opcional, ex FULL_SKILL.md)

Contiene el detalle completo (metodología paso a paso, ejemplos, advertencias). Se carga
bajo demanda, en el nivel 3 de progressive disclosure. `SKILL.md` debe quedar < 500 líneas.
...
```json
{
  ...
  "sources": [
    {
      "path": "arnes/skills/archive/<domain>/<recurso>.md",
      "extracted": "2026-08-05",
      "key_insights": ["<insight 1>", "<insight 2>"]
    }
  ],
  "aliases": ["<sinónimo 1>", "<sinónimo 2>"],
  "use_cases": ["<caso de uso 1>", "<caso de uso 2>"],
  "related_skills": ["<skill_relacionada>"],
  "maturity": "document",
