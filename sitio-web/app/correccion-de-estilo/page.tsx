...
export default async function CorreccionDeEstiloPage() {
  const testimonials = await getAllTestimonials();
  const portfolioItems = await getAuthorizedPortfolioItems();
  const featuredTestimonial =
    (await getFeaturedTestimonial()) ?? testimonials[0] ?? null;

  const secondaryTestimonial =
    testimonials.find(
      (t) => t.id !== (featuredTestimonial?.id ?? -1) && t.quote.length >= 70,
    ) ??
    testimonials.find((t) => t.id !== (featuredTestimonial?.id ?? -1)) ??
    null;
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-blue-pastel">
        ...
        <Container className="relative z-10 max-w-6xl py-20 sm:py-28 lg:py-32">
          ...
          <ul className="mt-16 flex flex-wrap items-center justify-center gap-x-10 gap-y-4 text-lg font-bold text-cream sm:text-xl">
      ...
      {/* B. Prueba social inmediata: franja terracota full width */}
      {featuredTestimonial && (
        <QuoteStrip
          quote={featuredTestimonial.quote}
          clientName={featuredTestimonial.clientName}
          bookTitle={featuredTestimonial.bookTitle}
        />
      )}
      ...
      {/* E. Segunda pausa de lectura: quote banner full width */}
      ...
      {/* F. Portafolio: slider sobre fondo lino/arena */}
      <section className="overflow-hidden bg-sand py-20 sm:py-24">
        <PortfolioSlider items={portfolioItems} testimonials={testimonials} />
      </section>
